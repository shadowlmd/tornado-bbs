...
type TWin32Obj = Object(TCommObj)
        ...
        procedure Com_ReadMax (Var Block; MaxLen: Longint; Var Reads: Longint); virtual;
        ...
        procedure Com_SetRts(State: Boolean); virtual;
        ...
        procedure Com_GetLine(var BpsRate: longint; var Parity: Char; var DataBits, Stopbits: Byte); virtual;
...
procedure TWin32Obj.Com_GetLine(var BpsRate: longint; var Parity: Char; var DataBits, Stopbits: Byte);
var DCB   : TDCB;
begin
  GetCommState(Com_GetHandle, DCB);

  BpsRate := dcb.BaudRate;
  ...
  Case dcb.StopBits of
     ...
     ONE5STOPBITS : StopBits := 3;
  else
    StopBits := 1;
  end; { case }

  DataBits := dcb.ByteSize;
end; { proc. TWin32Obj.Com_GetLine }
...
procedure TWin32Obj.Com_ReadMax(var Block; MaxLen: Longint; var Reads: Longint);
var
  tmp: Longint;
begin
  if InBuffer^.BufUsed < MaxLen then
    tmp := InBuffer^.BufUsed
  else
    tmp := MaxLen;
  CriticalRx^.EnterExclusive;
    Reads := InBuffer^.Get(Block, tmp, True);
  CriticalRx^.LeaveExclusive;
end; { proc. TWin32Obj.Com_ReadMax }
...
procedure TWin32Obj.Com_SetRts(State: Boolean);
begin
  if State then
    EscapeCommFunction(Com_GetHandle, SETRTS)
     else EscapeCommFunction(Com_GetHandle, CLRRTS);
end; { proc. TWin32Obj.Com_SetDtr }
