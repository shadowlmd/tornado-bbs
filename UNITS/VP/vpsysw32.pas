...
begin
  ...
  begin
    ...
    if StrLen(FindData.cAlternateFileName) = 0 then
    begin
      if IsPChar then
    ...
    begin
      if IsPChar then
        StrCopy(PChar(@Name), FindData.cAlternateFileName)
      else
        Name := StrPas(FindData.cAlternateFileName);
    end;
  end;
  Result := 0;
end;
...
begin
  Info.bVisible := Show;
  Size := Abs(Y1 - Y2);
  if Size <= 1   then Info.dwSize := 1   else
  if Size >= 100 then Info.dwSize := 100 else
    Info.dwSize := Size;
